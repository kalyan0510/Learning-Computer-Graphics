...
public:
           ...
           const char * fragShaderPath)
    {
        ...
        else
        {
            std::cout << std::unitbuf
                      << "[ERROR] " << __FILE__ << ':' << __LINE__ << ' ' << __PRETTY_FUNCTION__
                      << "\n[ERROR] " << "Tessellation control shader file not successfully read!"
                      << std::nounitbuf << std::endl;
        ...
        else
        {
            std::cout << std::unitbuf
                      << "[ERROR] " << __FILE__ << ':' << __LINE__ << ' ' << __PRETTY_FUNCTION__
                      << "\n[ERROR] " << "Tessellation evaluation shader file not successfully read!"
                      << std::nounitbuf << std::endl;
           ...
           const char * fragShaderPath)
    {
        ...
        if (std::ifstream fin {gsShaderPath, std::ifstream::in})
        {
            std::ostringstream sout;
            sout << fin.rdbuf();
            gsShaderCode = sout.str();
        }
        else
        {
            std::cout << std::unitbuf
                      << "[ERROR] " << __FILE__ << ':' << __LINE__ << ' ' << __PRETTY_FUNCTION__
                      << "\n[ERROR] " << "Geometry shader file not successfully read!"
                      << std::nounitbuf << std::endl;
        ...
        const char * ctrlShaderPtr = ctrlShaderCode.data();
        const char * evalShaderPtr = evalShaderCode.data();
        const char * gsShaderPtr = gsShaderCode.data();
        const char * fragShaderPtr = fragShaderCode.data();
        ...
        glShaderSource(gsShader, 1, &gsShaderPtr, nullptr);
    ...
    static void checkCompileErrors(unsigned int shader, const std::string & type)
    {
        ...
        else
        {
            ...
            if (!success)
            {
                ...
                std::cout << std::unitbuf
                          << "[ERRO]Rerror: linking with uncompiled/unspecialized shader " << __FILE__ << ':' << __LINE__ << ' ' << __PRETTY_FUNCTION__
